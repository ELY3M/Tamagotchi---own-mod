...
public abstract class Joint {
    ...
    public Vector2 getAnchorA() {
        ...
        this.anchorA.f3x = this.tmp[0];
        this.anchorA.f4y = this.tmp[1];
        return this.anchorA;
    }

    public Vector2 getAnchorB() {
        jniGetAnchorB(this.addr, this.tmp);
        this.anchorB.f3x = this.tmp[0];
        this.anchorB.f4y = this.tmp[1];
        return this.anchorB;
    }

    public Vector2 getReactionForce(float inv_dt) {
        jniGetReactionForce(this.addr, inv_dt, this.tmp);
        this.reactionForce.f3x = this.tmp[0];
        this.reactionForce.f4y = this.tmp[1];
