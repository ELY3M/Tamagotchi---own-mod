...
public class zzix {
    ...
    class C04251 implements Runnable {
        ...
        C04251(zzix com_google_android_gms_internal_zzix) {
            this.zzMI = com_google_android_gms_internal_zzix;
        }

        public void run() {
            synchronized (this.zzMI.zzpV) {
                zzin.m33v("Suspending the looper thread");
                while (this.zzMI.zzMH == 0) {
                    try {
                        this.zzMI.zzpV.wait();
                        zzin.m33v("Looper thread resumed");
                    } catch (InterruptedException e) {
                        zzin.m33v("Looper thread interrupted.");
                    }
                }
            }
        }
    ...
    public Looper zzhC() {
        ...
        synchronized (this.zzpV) {
            if (this.zzMH != 0) {
                ...
                zzin.m33v("Starting the looper thread.");
                this.zzMG = new HandlerThread("LooperProvider");
                this.zzMG.start();
                this.mHandler = new Handler(this.zzMG.getLooper());
                zzin.m33v("Looper thread started.");
            } else {
                zzin.m33v("Resuming the looper thread");
                this.zzpV.notifyAll();
            }
            this.zzMH++;
    ...
    public void zzhD() {
        synchronized (this.zzpV) {
            ...
            this.zzMH = i;
            if (i == 0) {
                this.mHandler.post(new C04251(this));
            }
        }
    }
}
