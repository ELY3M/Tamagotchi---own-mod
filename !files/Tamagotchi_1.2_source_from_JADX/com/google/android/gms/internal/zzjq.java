...
public class zzjq extends WebViewClient {
    ...
    class C04281 implements Runnable {
        ...
        C04281(zzjq com_google_android_gms_internal_zzjq) {
    ...
    private class zzd implements zzdf {
        ...
        public void zza(zzjp com_google_android_gms_internal_zzjp, Map<String, String> map) {
            if (map.keySet().contains("start")) {
                this.zzNL.zzij();
            } else if (map.keySet().contains(ISVGConstants.TAG_STOP)) {
                this.zzNL.zzik();
    ...
    public final void onLoadResource(WebView webView, String url) {
        zzin.m33v("Loading resource: " + url);
    ...
    public final boolean shouldOverrideUrlLoading(WebView webView, String url) {
        zzin.m33v("AdWebView shouldOverrideUrlLoading: " + url);
        Uri parse = Uri.parse(url);
        if ("gmsg".equalsIgnoreCase(parse.getScheme()) && "mobileads.google.com".equalsIgnoreCase(parse.getHost())) {
            zzh(parse);
    ...
    public final void zzfo() {
        synchronized (this.zzpV) {
            this.zzND = false;
            this.zzsz = true;
            zzir.runOnUiThread(new C04281(this));
        }
    }
    ...
    public void zzh(Uri uri) {
        ...
        if (list != null) {
            ...
            if (com.google.android.gms.ads.internal.util.client.zzb.zzQ(2)) {
                zzin.m33v("Received GMSG: " + path);
                for (String path2 : zze.keySet()) {
                    zzin.m33v("  " + path2 + ": " + ((String) zze.get(path2)));
                }
            }
            for (zzdf zza : list) {
        ...
        zzin.m33v("No GMSG handler found for GMSG: " + uri);
    }
    ...
    public void zzii() {
        synchronized (this.zzpV) {
            zzin.m33v("Loading blank page in WebView, 2...");
            this.zzNH = true;
