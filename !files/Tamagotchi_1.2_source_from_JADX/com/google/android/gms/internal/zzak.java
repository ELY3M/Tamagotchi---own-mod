...
public abstract class zzak implements zzaj {
    ...
    String zza(byte[] bArr, String str) throws NoSuchAlgorithmException, UnsupportedEncodingException, IOException {
        ...
        if (bArr.length < 239) {
            ...
            bArr2 = ByteBuffer.allocate(ColorUtils.COLOR_MASK_12BIT_RGB_G).put((byte) bArr.length).put(bArr).put(bArr2).array();
        } else {
            bArr2 = ByteBuffer.allocate(ColorUtils.COLOR_MASK_12BIT_RGB_G).put((byte) bArr.length).put(bArr).array();
        }
